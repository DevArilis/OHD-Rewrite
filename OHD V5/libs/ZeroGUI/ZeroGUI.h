...
inline CG::UFont* get_font()
{
	static CG::UFont* font;
	if (!font)
		font = CG::UObject::FindObject<CG::UFont>("Font RobotoDistanceField.RobotoDistanceField");

	return font;
}

inline CG::UFont* fonts;
...
namespace ZeroGUI
{
	namespace Colors
	{
		...
		inline auto Window_Header = rgb2rgbfl(255, 0, 0);
	...
	namespace PostRenderer
	{
		...
		inline void TextLeft(char* name, FVector2D pos, FLinearColor color, bool outline)
		{
			for (int i = 0; i < 128; i++)
			{
				if (drawlist[i].type == -1)
		...
		inline void TextCenter(char* name, FVector2D pos, FLinearColor color, bool outline)
		{
			for (int i = 0; i < 128; i++)
			{
				if (drawlist[i].type == -1)
	...
	inline FVector2D menu_pos = FVector2D{ 0, 0 };
	inline float offset_x = 0.0f;
	inline float offset_y = 0.0f;

	inline FVector2D first_element_pos = FVector2D{ 0, 0 };

	inline FVector2D last_element_pos = FVector2D{ 0, 0 };
	inline FVector2D last_element_size = FVector2D{ 0, 0 };

	inline int current_element = -1;
	inline FVector2D current_element_pos = FVector2D{ 0, 0 };
	inline FVector2D current_element_size = FVector2D{ 0, 0 };
	inline int elements_count = 0;

	inline bool sameLine = false;

	inline bool pushY = false;
	inline float pushYvalue = 0.0f;
	inline bool pushX = false;
	inline float pushXvalue = 0.f;

	inline void SetupCanvas(UCanvas* _canvas)
	{
		canvas = _canvas;
		fonts = get_font();
	...
	inline void TextLeft(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		canvas->K2_DrawText(fonts, FString{ s2wc(name) }, pos, FVector2D{ 0.5f, 0.5f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	inline void TextCenter(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		canvas->K2_DrawText(fonts, FString{ s2wc(name) }, pos, FVector2D{ 0.5f, 0.5f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}

	inline void GetColor(FLinearColor* color, float* r, float* g, float* b, float* a)
	{
		*r = color->R;
		*g = color->G;
		*b = color->B;
	...
	inline UINT32 GetColorUINT(int r, int g, int b, int a)
	{
	...
	inline void DrawCircle(FVector2D pos, int radius, int numSides, FLinearColor Color)
	{
		float PI = 3.1415927f;
	...
	inline FVector2D dragPos;
	inline bool Window(char* name, FVector2D* pos, FVector2D size, bool isOpen)
	{
		elements_count = 0;
	...
	inline void Text(char* text, bool center = false, bool outline = false)
	{
		elements_count++;
	...
	inline bool ButtonTab(char* name, FVector2D size, bool active)
	{
		elements_count++;
	...
	inline bool Button(char* name, FVector2D size)
	{
		elements_count++;
		...
		if (isHovered)
		{
			drawFilledRect(FVector2D{ pos.X, pos.Y - 4 }, size.X, size.Y, Colors::Button_Hovered);
			hover_element = true;
		}
		else
		{
			drawFilledRect(FVector2D{ pos.X, pos.Y - 4 }, size.X, size.Y, Colors::Button_Idle);
		}
		...
		//Text
		FVector2D textPos = FVector2D{ pos.X + size.X / 2, (pos.Y - 4) + size.Y / 2};
	...
	inline void Checkbox(char* name, bool* value)
	{
		elements_count++;
	...
	inline void SliderInt(char* name, int* value, int min, int max)
	{
		elements_count++;
		...
		FVector2D pos = FVector2D{ menu_pos.X + padding.X + offset_x, (menu_pos.Y - 6) + padding.Y + offset_y };
		if (sameLine)
		{
			pos.X = last_element_pos.X + last_element_size.X + padding.X;
			pos.Y = last_element_pos.Y;
	...
	inline void SliderFloat(char* name, float* value, float min, float max, char* format = (char*)"%.0f")
	{
		elements_count++;
	...
	inline bool checkbox_enabled[256];
	inline void Combobox(char* name, FVector2D size, int* value, const char* arg, ...)
	{
		elements_count++;
	...
	inline int active_hotkey = -1;
	inline bool already_pressed = false;
	...
	inline char* VirtualKeyCodeToChar(UCHAR virtualKey)
	{
		UINT scanCode = MapVirtualKey(virtualKey, MAPVK_VK_TO_VSC);
		...
		if (virtualKey == VK_XBUTTON1)
		{
			return (char*)("XBUTTON1");
		}
		if (virtualKey == VK_XBUTTON2)
		{
			return (char*)("XBUTTON2");
		}
	...
	inline void Hotkey(char* name, FVector2D size, int* key)
	{
		elements_count++;
	...
	inline int active_picker = -1;
	inline FLinearColor saved_color;
	inline bool ColorPixel(FVector2D pos, FVector2D size, FLinearColor* original, FLinearColor color)
	{
	...
	inline void ColorPicker(char* name, FLinearColor* color)
	{
		elements_count++;
		...
		if (sameLine)
		{
			pos.X = last_element_pos.X + last_element_size.X - 90.f;
			pos.Y = last_element_pos.Y;
		}
		if (pushY)
		{
		...
		{
			//Bg
			DrawFilledCircle(FVector2D{ pos.X + 16, pos.Y + 12 }, 10.0f, Colors::ColorPicker_Background);

			

			//Color
			DrawFilledCircle(FVector2D{ pos.X + 16, pos.Y + 12 }, 6.0f, * color);
